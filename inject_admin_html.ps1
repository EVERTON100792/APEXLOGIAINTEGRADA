  ...
  <div class="acc-container">
    ...
    <div id="acc-pane-regras" class="acc-pane">
      ...
      <div class="acc-grid-2 acc-mt">
        ...
        <div class="acc-card">
          <div class="acc-card-header-row">
            <h3 class="acc-card-title"><i class="bi bi-chat-square-quote-fill me-2" style="color: #f59e0b;"></i>Observações de Clientes</h3>
          </div>
          <p class="acc-hint">Notas exibidas na parte inferior dos cards de carga (cargas automáticas, especiais e manuais).</p>
          <div class="acc-form-group">
            <input id="acc-obs-code" class="acc-input" placeholder="Código do Cliente (ex: 4277)" style="margin-bottom:0.5rem">
            <textarea id="acc-obs-text" class="acc-textarea" placeholder="Observação (ex: Não abre de segunda / Não paga descarga)" style="height:60px"></textarea>
            <div style="display: flex; gap: 8px; margin-top: 8px;">
              <button type="button" class="acc-btn acc-btn-primary" style="flex: 2; height: 42px; font-size: 0.88rem; font-weight: bold; display: flex; align-items: center; justify-content: center; gap: 6px; cursor: pointer; border-radius: 6px;" onclick="accSalvarObsCliente()">
                <i class="bi bi-check-circle-fill"></i> SALVAR OBSERVAÇÃO
              </button>
              <button type="button" class="acc-btn acc-btn-danger" style="flex: 1; height: 42px; font-size: 0.88rem; font-weight: bold; display: flex; align-items: center; justify-content: center; gap: 6px; cursor: pointer; border-radius: 6px;" onclick="accExcluirObsPorFormulario()" title="Excluir a observação do código digitado acima">
                <i class="bi bi-trash-fill"></i> EXCLUIR
              </button>
            </div>
          </div>
          <div style="margin-top: 14px; margin-bottom: 6px;">
            <input id="acc-obs-search" class="acc-input" placeholder="🔍 Buscar por código ou texto de observação..." style="font-size: 0.8rem; padding: 0.4rem 0.7rem; border-radius: 6px;" oninput="accFiltrarObs(this.value)">
          </div>
          <div class="acc-table-scroll acc-mt" style="max-height: 380px; overflow-y: auto; overflow-x: hidden;">
            <table class="acc-table" style="table-layout: fixed; width: 100%;">
              <thead><tr><th style="width: 70px;">Cod</th><th>Observação</th><th style="width: 95px; text-align: center;">Ação</th></tr></thead>
